...
def add_kinetic_on_label(kinetic_file,synth_file,seq_file,outfile):
    kinetic_data = pd.read_csv(kinetic_file,names=['kinetic_ID'])
    synth_data = pd.read_csv(synth_file,names=['syn_ID'])
    ...
    lines_kin = uclust_data.loc[uclust_data.kinetic_flag == '1','Target'].apply(lambda x: x.split(' ')[0]).values + ',label,node,#B2BABB,1,normal\n'
    lines_syn = uclust_data.loc[uclust_data.syn_flag == '1','Target'].apply(lambda x: x.split(' ')[0]).values + ',label,node,#BA4A00,1,normal\n'
    print(lines_kin)
    lines = pd.np.concatenate([lines_kin,lines_syn])
